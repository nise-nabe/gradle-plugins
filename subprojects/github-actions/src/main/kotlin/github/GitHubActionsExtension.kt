...
abstract class GitHubActionsExtension(
     ...
     */
    val workflow: Provider<String> = env(GITHUB_WORKFLOW)

    /**
     * A unique number for each run within a repository. This number does not change if you re-run the workflow run.
     */
    val runId: Provider<String> = env(GITHUB_RUN_ID)

    /**
     * A unique number for each run of a particular workflow in a repository. This number begins at 1 for the workflow's first run, and increments with each new run. This number does not change if you re-run the workflow run.
     */
    val runNumber: Provider<String> = env(GITHUB_RUN_NUMBER)

    /**
     * The job_id of the current job.
     */
    val job: Provider<String> = env(GITHUB_JOB)

    /**
     * The unique identifier (id) of the action.
     */
    val action: Provider<String> = env(GITHUB_ACTION)
     ...
     */
    val actionPath: Provider<String> = env(GITHUB_ACTION_PATH)
     ...
     */
    val actions: Provider<String> = env(GITHUB_ACTIONS)

    /**
     * The name of the person or app that initiated the workflow. For example, octocat.
     */
    val actor: Provider<String> = env(GITHUB_ACTOR)

    /**
     * The owner and repository name. For example, `octocat/Hello-World.`
     */
    val repository: Provider<String> = env(GITHUB_REPOSITORY)

    /**
     * The name of the webhook event that triggered the workflow.
     */
    val eventName: Provider<String> = env(GITHUB_EVENT_NAME)

    /**
     * The path of the file with the complete webhook event payload. For example, /github/workflow/event.json.
     */
    val eventPath: Provider<String> = env(GITHUB_EVENT_PATH)
     ...
     */
    val runnerTemp: Provider<String> = env(RUNNER_TEMP)
     ...
     */
    val runnerToolCache: Provider<String> = env(RUNNER_TOOL_CACHE)

    private fun env(env: GithubActionsEnv): Provider<String> {
        return providers.environmentVariable(env.name)
    }
}
